# from src.widget import mask_account_card, get_date
# from src.processing import filter_by_state, sort_by_date
# from src.generators import filter_by_currency, transaction_descriptions, card_number_generator
#
# dictionary_id = [
#     {"id": 41428829, "state": "EXECUTED", "date": "2019-07-03T18:35:29.512364"},
#     {"id": 939719570, "state": "EXECUTED", "date": "2018-06-30T02:08:58.425572"},
#     {"id": 594226727, "state": "CANCELED", "date": "2018-09-12T21:27:25.241689"},
#     {"id": 615064591, "state": "CANCELED", "date": "2018-10-14T08:21:33.419441"},
# ]
#
#
# transactions = [
#     {
#         "id": 939719570,
#         "state": "EXECUTED",
#         "date": "2018-06-30T02:08:58.425572",
#         "operationAmount": {"amount": "9824.07", "currency": {"name": "USD", "code": "USD"}},
#         "description": "Перевод организации",
#         "from": "Счет 75106830613657916952",
#         "to": "Счет 11776614605963066702",
#     },
#     {
#         "id": 142264268,
#         "state": "EXECUTED",
#         "date": "2019-04-04T23:20:05.206878",
#         "operationAmount": {"amount": "79114.93", "currency": {"name": "USD", "code": "USD"}},
#         "description": "Перевод со счета на счет",
#         "from": "Счет 19708645243227258542",
#         "to": "Счет 75651667383060284188",
#     },
#     {
#         "id": 873106923,
#         "state": "EXECUTED",
#         "date": "2019-03-23T01:09:46.296404",
#         "operationAmount": {"amount": "43318.34", "currency": {"name": "руб.", "code": "RUB"}},
#         "description": "Перевод со счета на счет",
#         "from": "Счет 44812258784861134719",
#         "to": "Счет 74489636417521191160",
#     },
#     {
#         "id": 895315941,
#         "state": "EXECUTED",
#         "date": "2018-08-19T04:27:37.904916",
#         "operationAmount": {"amount": "56883.54", "currency": {"name": "USD", "code": "USD"}},
#         "description": "Перевод с карты на карту",
#         "from": "Visa Classic 6831982476737658",
#         "to": "Visa Platinum 8990922113665229",
#     },
#     {
#         "id": 594226727,
#         "state": "CANCELED",
#         "date": "2018-09-12T21:27:25.241689",
#         "operationAmount": {"amount": "67314.70", "currency": {"name": "руб.", "code": "RUB"}},
#         "description": "Перевод организации",
#         "from": "Visa Platinum 1246377376343588",
#         "to": "Счет 14211924144426031657",
#     },
# ]
#
# # transactions = []
# number = ""
# print("Введите тип и номер карты или введите счет:")
# user_card_or_score = input()
# print(mask_account_card(user_card_or_score))
#
# print("Введите дату:")
# user_date = input()
# print(get_date(user_date))
#
# print("Введите параметр для ключа состояния операций state, успешные или отмененные (EXECUTED/CANCELED):")
# state = input()
# print(filter_by_state(dictionary_id, state))
#
# print("Введите метод сортировки операций, по убыванию или по возрастанию (True/False):")
# ascending = input()
# print(sort_by_date(dictionary_id, ascending))
#
# print("Описание каждой операции по всем транзакциям по очереди:")
# # if transactions != []:
# #     descriptions = transaction_descriptions(transactions)
# #     for i in range(len(transactions)):
# #         print(next(descriptions))
# # else:
# #     transaction_descriptions(transactions)
# descriptions = transaction_descriptions(transactions)
# for i in range(len(transactions)):
#     print(next(descriptions))
#
# print("Генерация номеров банковских карт в формате XXXX XXXX XXXX XXXX:")
# for card_number in card_number_generator(1, 5):
#     print(card_number)
#
# print("Введите валюту операции для вывода транзакций:")
# user_currency = input()
# user_transactions = filter_by_currency(transactions, user_currency)
# for i in range(2):
#     print(next(user_transactions))


from typing import Any

from src.main_functions import number, status, ad_questions
from src.widget import get_date, mask_account_card


def main() -> Any:
    """Главная функция проекта"""
    modul = number()
    # print(modul)
    modul_transaction = status(modul)
    # print(modul_transaction)
    final = ad_questions(modul_transaction)
    if not final:
        return "Программа: Не найдено ни одной транзакции, подходящей под ваши условия фильтрации"
    else:
        print("Программа: Распечатываю итоговый список транзакций...")
        print(f"Программа: \nВсего банковских операций в выборке: {len(final)}\n")
        for fina in final:
            print(f'{get_date(fina.get("date"))} {fina.get("description")}')
            if fina.get("description") == "Открытие вклада":
                print(f'{mask_account_card(fina.get("to"))}')
                print(
                    f'Сумма: {fina.get("operationAmount").get("amount")} '
                    f'{fina.get("operationAmount").get("currency").get("code")}\n'
                )
            else:
                print(f'{mask_account_card(fina.get("from"))} -> {mask_account_card(fina.get("to"))}')
                print(
                    f'Сумма: {fina.get("operationAmount").get("amount")} '
                    f'{fina.get("operationAmount").get("currency").get("code")}\n'
                )
        return "конец"


if __name__ == "__main__":
    print(main())
